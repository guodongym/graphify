...
def build_code_graph(
    *,
    ...
    _extract_elapsed_ms = int((perf_counter() - _extract_start) * 1000)
    ...
    if tabular_manifest is not None:
        ...
        trace.record_stage_ms("code_extract_ms", _extract_elapsed_ms)
        ...
        if should_update_sidecar:
            try:
                ...
                if has_sidecar_projection:
                    ...
                    trace.record_stage_ms("sidecar_projection_ms", int((perf_counter() - proj_start) * 1000))
    ...
    try:  # outer-try: covers post-sidecar build finalization for failed-trace
        ...
        _graph_build_start = perf_counter()
        graph = build_from_json(result, root=repo_root)
        if trace is not None:
            trace.record_stage_ms("graph_build_ms", int((perf_counter() - _graph_build_start) * 1000))
        ...
        _cluster_start = perf_counter()
        communities = cluster(graph)
        cohesion = score_all(graph, communities)
        gods = god_nodes(graph)
        surprises = surprising_connections(graph, communities)
        if trace is not None:
            trace.record_stage_ms("cluster_ms", int((perf_counter() - _cluster_start) * 1000))
        ...
        if trace is not None:
            trace.record_stage_ms("report_export_ms", int((perf_counter() - _report_start) * 1000))
