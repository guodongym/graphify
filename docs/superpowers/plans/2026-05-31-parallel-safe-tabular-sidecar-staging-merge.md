...
def test_build_trace_writes_success_json(tmp_path):
    ...
    trace.record_stage_ms("sidecar_stage_write_ms", 12)
    ...
    assert payload["stages"]["sidecar_stage_write_ms"] == 12
    assert "tabular_parse_ms" not in payload["stages"]
    assert "sidecar_merge_wait_ms" not in payload["stages"]
...
def test_build_trace_writes_failed_json(tmp_path):
    ...
    trace.record_stage_ms("sidecar_merge_write_ms", 5)
    ...
    assert payload["stages"]["sidecar_merge_write_ms"] == 5
```
